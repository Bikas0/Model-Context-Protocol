...
async def classify_companies():
    try:
        ...
        results = []  # Instead of DataFrame, use list of dicts first

        company_types = [
            "Wholesale Distribution", "Manufacturing", "Asset-Based Lending",
            "Freight & Transportation", "Business Services",
            "Transportation and Lodging", "Business and Professional Services",
            "Engineering & Construction"
        ]
        
        api_keys = [
            "AIzaSyAvFmMs0bZ_ujBI0bTeQJCZ7uP0WethAlc", "AIzaSyDvu8IL0taaKQD1tRBa-dLCmsLQZSdQqHk",
            "AIzaSyDASnL-ZxTMDlprPnYHUV8Oj7OOq6qOaME", "AIzaSyDTnWWoyjCZcsrMrk9AVVw1YKNPy-xmiSk",
            "AIzaSyCSoVtf8OPpjqa3bs2ZeLCVBExqZ1qapFM", "AIzaSyDy-Q4yt5UWo11GFhRyT96VxY3s-gsL97c",
            "AIzaSyD2WqDCIqb0-8umU4MxP4pkvvgLsPZmzDI", "AIzaSyAuvqB2UcAtj_kdSfE1Xt3ATFMopPe_pPI",
            "AIzaSyATgA0W8WgLzPZV5zczeHvdCitZUnmg0os", "AIzaSyBYowZjz3yfoM1DCwz1XtE91H5YouC3wVY"
        ]
        ...
        for idx, row in df.iterrows():
            ...
            # Rotate API key
            current_api_key = api_keys[idx % len(api_keys)]
            llm = ChatGoogleGenerativeAI(
                model="gemini-2.0-flash",
                google_api_key=current_api_key
            )

            # Create agent (small creation compared to MCP client)
            agent = MCPAgent(
                llm=llm,
                client=mcp_client,
                max_steps=15,
            ...
            try:
                ...
                if match:
                    final_answer = match.group(1)
                print("-"*50)
                print(f"[{idx+1}/{total_companies}] {company_name}: {final_answer}")
                ...
                print(f"[{idx+1}/{total_companies}] Error with {company_name}: {e}")
                results.append({
                    'Company Name': company_name,
                    'Company Type': 'others',
            ...
            if (idx + 1) % 10 == 0:
                pd.DataFrame(results).to_csv('company_classifications.csv', index=False)
                print(f"Saved checkpoint at {idx+1} companies.")
                await asyncio.sleep(1)  # Optional tiny break
